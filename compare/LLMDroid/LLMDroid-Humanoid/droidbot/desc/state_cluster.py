...
class StateCluster(ActionListener):
    ...
    def to_json(self):
        ret = {
            "id": self.__id,
            "root": f"State{self.__root_state.get_id()}",
            "states": [state.get_id() for state in self.__states],
    ...
    def update_tested_function(self, function: str):
        ...
        with self.__lock:
            if function in self.__functions:
                ...
                self.__functions[function] = FunctionDetail(widget_id=-1, importance=0)
    ...
    def __set_function_to_widget(self, function_list):
        for function in function_list.keys():
            ...
            if widget:
                ...
                self.logger.warning(f"({function}:{widget_id}) can't find widget in Root State{self.__root_state.get_id()}")
    ...
    def __update_later_joined_state(self, state: 'DeviceState'):
        ...
        for widget in self.__root_state.get_all_widgets():
            ...
            if target:
                ...
                self.logger.info(f"Successfully set function:{function} to widget({target.to_html()[:-1]}) in State{state.get_id()}")
            else:
