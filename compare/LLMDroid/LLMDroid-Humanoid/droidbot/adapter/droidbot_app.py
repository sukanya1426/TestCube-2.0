...
class DroidBotAppConn(Adapter):
    ...
    def set_up(self):
        ...
        if DROIDBOT_APP_PACKAGE in device.adb.get_installed_apps():
            self.logger.info("DroidBot app was already installed.")
        else:
            # install droidbot app
            self.logger.info("try to install droidbot app")
            try:
        ...
        if DROIDBOT_APP_PACKAGE not in device.adb.get_installed_apps():
            self.logger.warning("install still failed")
