...
public class MonkeySourceApeNative implements MonkeyEventSource {
     ...
     */
    public void initReuseAgent(boolean useCodeCoverage) {
        AiClient.InitAgent(AiClient.AlgorithmType.Reuse, this.packageName, useCodeCoverage);
    }
            ...
            try {
                ...
                if (saveGUITreeToXmlEveryStep) {
                    ...
                    File xmlFile = new File(checkOutputDir(), String.format(stringFormatLocale,
                            "step-%d.xml", timeStep));
                    Logger.println(String.format(stringFormatLocale, "Saving GUI tree to %s at step %d",
                            xmlFile.getName(), timeStep));
                ...
                // Create a simple date format object and set the date and time format
                SimpleDateFormat dateFormat = (SimpleDateFormat) DateFormat.getDateTimeInstance();
                dateFormat.applyPattern("MM-dd_HH_mm_ss_SSS");
                // Format timestamp using simple date format
                String readableDate = dateFormat.format(new Date(timeMillis));

                if (takeScreenshotForEveryStep) {
                    checkOutputDir();
                    File screenshotFile = new File(checkOutputDir(), String.format(stringFormatLocale,
                            "step-%d-%s-%s-%s.png", timeStep, sid, aid, readableDate));
