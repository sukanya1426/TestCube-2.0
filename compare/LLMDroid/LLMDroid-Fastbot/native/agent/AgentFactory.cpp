...
/// \return
    AbstractAgentPtr
    AgentFactory::create(AlgorithmType agentT, const ModelPtr &model, bool useCodeCoverage, DeviceType deviceType) {
        AbstractAgentPtr agent = nullptr;
        // use ModelReusableAgent under all circumstances.
        ReuseAgentPtr reuseAgent = std::make_shared<ModelReusableAgent>(model, useCodeCoverage);
