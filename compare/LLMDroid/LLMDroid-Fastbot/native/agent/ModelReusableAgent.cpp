...
namespace fastbotx {
    ...
    ModelReusableAgent::ModelReusableAgent(const ModelPtr &model, bool useCodeCoverage)
            : AbstractAgent(model, useCodeCoverage),
            _alpha(SarsaRLDefaultAlpha),
            _epsilon(SarsaRLDefaultEpsilon),
...
/// If the new action is generated,
    ...
    ActionPtr ModelReusableAgent::selectNewAction() {
        ...
        // use action->getVisitedCount() to check if an action is visited
        action = this->selectUnperformedActionNotInReuseModel();
        if (nullptr != action) {
            MLOG("select action not in reuse model");
            return action;
        }

        action = this->selectUnperformedActionInReuseModel();
        if (nullptr != action) {
            MLOG("select action in reuse model");
            return action;
        }

        action = this->_newState->randomPickUnvisitedAction();
        if (nullptr != action) {
            MLOG("select action in unvisited action");
            return action;
        }

        // ADD: ask gpt for action
        MLOG("%s", "Ask gpt for action");
        action = askGPTForAction();
        if (nullptr != action) {
            MLOG("%s", "select action from gpt");
            return action;
        }
        ...
        if (nullptr != action) {
            MLOG("%s", "select action by qvalue");
            return action;
        }
        ...
        if (nullptr != action) {
            MLOG("%s", "select action by EpsilonGreedyRandom");
            return action;
        }
    ...
    ActionPtr ModelReusableAgent::askGPTForAction() 
    {
        // 生成之前测试过程的flowchart
        //std::string flowchart = this->_graph->generateGraphCode();
        //MLOG("ModelReusableAgent: state size in _graph: %d", (int)this->_graph->stateSize());
        //MLOG("===FLOWCHART===\n%s", flowchart.c_str());
        
        // 生成当前state描述
        // ReuseStatePtr reuseState = std::dynamic_pointer_cast<ReuseState>(this->_newState);
        // std::string stateDescription = reuseState->getStateDescription();
        // MLOG("%s", stateDescription.c_str());
        // TODO: 与gpt对话，得到动作序号，从_newState->getActions()得到action并返回

        // 文件路径
        //std::string filePath = "/sdcard/flowchart.txt";
        // 打开文件并清空内容
        //std::ofstream outputFile(filePath, std::ofstream::out | std::ofstream::trunc);
        // if (outputFile.is_open()) {
        //     // 写入字符串内容
        //     outputFile << stateDescription << 
        //         this->_graph->_firstState->getStateDescription() << "\n";
        //         //this->_graph->generateGraphCodeForActivity();
        //     // 关闭文件
        //     outputFile.close();
        // }else {
        //     MLOG("write file error");
        // }
