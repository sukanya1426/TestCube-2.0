    ...
    int State::mergeWidgetAndStoreMergedOnes(WidgetPtrSet &mergeWidgets) {
        ...
        if (STATE_MERGE_DETAIL_TEXT && !this->_widgets.empty()) {
            for (const auto &widgetPtr: this->_widgets) {
                ...
                // If the addition is successful, _mergedWidgets will no longer be added, which means that the first widget is not in _mergedWidgets.
                // Failed to join, the element already exists, add it to _mergedWidgets
                if (!noMerged) {
                    uintptr_t h = widgetPtr->hash();
                    mergedWidgetCount++;
                    // Check whether the widget already exists in map<hash, vector<widget>>
                    ...
                    } else {// If there is any, add it to the original vector
                        // Because there may be multiple widgets whose hashes are calculated to be the same, use vector to store them.
