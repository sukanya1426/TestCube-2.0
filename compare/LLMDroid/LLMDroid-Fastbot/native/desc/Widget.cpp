...
namespace fastbotx {
    ...
    void Widget::initFormElement(const ElementPtr &element) {
        ...
        // set _info to text or content-desc
        if (!this->_text.empty()) {
            this->_info = this->_text;
        }
        else if (!this->_contextDesc.empty()) {
            this->_info = this->_contextDesc;
        }
        ...
        uintptr_t hashcode5 = std::hash<int>{}(this->_bounds->right - this->_bounds->left);
        uintptr_t hashcode6 = std::hash<int>{}(this->_bounds->bottom - this->_bounds->top);

        this->_hashcode = ((hashcode1 ^ (hashcode2 << 4)) >> 2) ^
                          (((127U * hashcode3 << 1) ^ (256U * hashcode4 << 3)) >> 1);
        this->_myHashcode = ((hashcode1 ^ (hashcode2 << 5)) >> 3) ^
                          (((127U * hashcode3 << 1) ^ (256U * hashcode4 << 4)) >> 2) ^
    ...
    Widget::~Widget() {
        MLOG("widget is about to be destroyed: class %s res-id: %s", 
            this->_clazz.c_str(), this->_resourceID.c_str());
        this->_actions.clear();
    ...
    std::string Widget::getResourceID()
    {
        size_t pos = this->_resourceID.find_last_of('/');
        if (pos != std::string::npos) {
            // Extract the substring starting from the character after '/'
            return this->_resourceID.substr(pos + 1);
        }
        else {
    ...
    std::string Widget::getClass()
    {
        size_t pos = this->_clazz.find_last_of('.');
        if (pos != std::string::npos) {
            // Extract the substring starting from the character after '/'
            return this->_clazz.substr(pos + 1);
        }
        else {
            return this->_clazz;
        }        
    ...
    std::string Widget::getDescriptionInfo()
    {
        std::string info;
        info = _text;
        if (info.empty()) {
