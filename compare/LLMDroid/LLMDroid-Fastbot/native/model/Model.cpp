...
#define DefaultDeviceID "0000001"
    ...
    AbstractAgentPtr Model::addAgent(const std::string &deviceIDString, AlgorithmType agentType,
        ...
        auto agent = AgentFactory::create(agentType, shared_from_this(), useCodeCoverage, deviceType);
    ...
    OperatePtr Model::getOperateOpt(const ElementPtr &element, const std::string &activity,
        ...
        if (this->_deviceIDAgentMap.empty())  // create a default agent
        {
            ...
            this->addAgent(DefaultDeviceID, AlgorithmType::Reuse, false);
        }
        ...
        if (nullptr != element) // make sure the XML is not null
        {
            ...
            ReuseStatePtr reuseState = std::dynamic_pointer_cast<ReuseState>(state);
        ...
        //callJavaLogger(MAIN_THREAD, state->toString().c_str());
        ...
        if (nullptr == customActionPtr && !shouldSkipActionsFromModel) {
            if (-1 != BLOCK_STATE_TIME_RESTART &&
                ...
                callJavaLogger(MAIN_THREAD, "[MAIN] after resolveNewAction: %s", action->toDescription().c_str());
                //MLOG("[Action after resolveNewAction]: %s", action->toDescription().c_str());
            ...
            // check whether action's type is among BACK~SCROLL_BOTTOM_UP_N
        ...
        if (action != nullptr) {
            ...
            if (state)
            {
                ReuseStatePtr tmp = std::dynamic_pointer_cast<ReuseState>(state);
                tmp->_actionToPerform = action;
                MLOG("[Set state%d actionToPerform]: %s", state->getIdi(), tmp->_actionToPerform->toDescription().c_str());
            }

            // If there is no input content before the action, it will be randomly generated.
            if (!action->hasInput() && this->_preference) {
                std::string inputText = this->_preference->patchOperate(opt);
                if (!inputText.empty()) {
                    std::string origin = action->toDescription();
                    action->setInputText(inputText);
                    callJavaLogger(MAIN_THREAD, "[INPUT] act: %s to %s", origin.c_str(), action->toDescription().c_str());
                }
            }
            // If the action has specified input content, the previous input will be used.
            else {
                opt->setText(action->getInputText());
            }
