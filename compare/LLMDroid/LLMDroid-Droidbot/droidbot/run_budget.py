...
LLMDroid and TestCube are both measured over that same hour. This file is kept
byte-for-byte parallel with TestCube's ``droidbot/run_budget.py`` so the two
tools cannot be cut off on different terms.
...
* the **soft** deadline, checked at the top of the policy loop and again in
  ...
  Crossing it raises ``InputInterruptedException``, so the run unwinds along
  its normal shutdown path and the final coverage sample is taken.
