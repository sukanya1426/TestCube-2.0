...
class TraceLoader(object):
    ...
    def _visible_texts(self, state):
        ...
        if isinstance(views, list):
            ...
            # LLMDroid's DeviceState.to_dict() stores an HTML-ish widget
            # dump under "views" instead of a list of view dicts. Pull
            # quoted attribute values and inner text nodes out of it so
            # the coverage judge still gets on-screen labels as evidence.
            texts.extend(re.findall(r'(?:text|desc|content-desc)="([^"]{1,120})"', views))
            texts.extend(
                fragment.strip()
                for fragment in re.findall(r">([^<>{}]{1,120})<", views)
                if fragment.strip()
            )
