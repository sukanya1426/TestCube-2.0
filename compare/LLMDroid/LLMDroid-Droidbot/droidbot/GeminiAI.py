...
try:
    # Optional: only needed for the Gemini fallback backend. This port is
    # used with LLM_BACKEND=local (Ollama) by default and must not require
    # installing anything just to import this module.
...
class GeminiAi:
    ...
    def _ensure_model(cls):
        if genai is None:
            raise RuntimeError(
                "google-generativeai is not installed; only LLM_BACKEND=local (Ollama) is available"
            )
    ...
    def _generate_gemini(cls, prompt, timeout=45):
        ...
        if genai is None:
            raise RuntimeError("google-generativeai is not installed; Gemini backend unavailable")
        last_error = None
        for name in GEMINI_MODEL_NAMES:
            model_usable = False
